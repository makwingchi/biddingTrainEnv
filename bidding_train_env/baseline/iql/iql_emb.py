...
class IQL:
    ...
    def __init__(self, dim_obs=3, dim_actions=1, budget_emb_dim=3, category_emb_dim=5,
                 gamma=0.99, tau=0.01, V_lr=1e-4, critic_lr=1e-4, actor_lr=1e-4,
                 network_random_seed=1, expectile=0.7, temperature=3.0):
        self.budget_embedding_dim = budget_emb_dim
        self.category_embedding_dim = category_emb_dim

        self.num_of_states = dim_obs + self.budget_embedding_dim - 1 #+ self.category_embedding_dim
        ...
        self.budget_embedding = torch.nn.Embedding(6, self.budget_embedding_dim)
        self.category_embedding = torch.nn.Embedding(5, self.category_embedding_dim)

        self.budget_embedding_optimizer = Adam(
            self.budget_embedding.parameters(),
            lr=max(self.V_lr, self.critic_lr, self.actor_lr)
        )

        # self.budget_embedding = torch.nn.Linear(in_features=1, out_features=self.budget_embedding_dim)
        # self.category_embedding = torch.nn.Linear(in_features=1, out_features=self.category_embedding_dim)

    def step(self, states, actions, rewards, next_states, dones):
        '''
        训练网络
        '''
        # extract budget embedding
        curr_budgets = states[:, 0].long()
        curr_budget_emb = self.budget_embedding(curr_budgets)
        next_budgets = next_states[:, 0].long()
        next_budget_emb = self.budget_embedding(next_budgets)

        # print(curr_budgets)
        # print(curr_budget_emb)
        # print(next_budget_emb)

        # extract agent category embedding
        # curr_agent_categories = states[:, 1].long()
        # curr_agent_category_emb = self.category_embedding(curr_agent_categories)
        # next_agent_categories = next_states[:, 1].long()
        # next_agent_category_emb = self.category_embedding(next_agent_categories)

        # concat
        states = torch.cat([curr_budget_emb, states[:, 1:]], dim=-1)
        next_states = torch.cat([next_budget_emb, next_states[:, 1:]], dim=-1)

        value_loss = self.calc_value_loss(states, actions)
        value_loss.backward(retain_graph=True)
        self.value_optimizer.step()
        self.value_optimizer.zero_grad()
        # self.budget_embedding_optimizer.step()
        # self.budget_embedding_optimizer.zero_grad()
    ...
    def take_actions(self, states):
        ...
        输出动作
        '''
        budgets = states[:, 0].long()
        budget_emb = self.budget_embedding(budgets)

        # agent_categories = states[:, 1].long()
        # agent_category_emb = self.category_embedding(agent_categories)

        states = torch.cat((budget_emb, states[:, 1:]), dim=-1)
