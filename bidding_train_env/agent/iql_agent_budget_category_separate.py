...
class IqlAgent(BaseAgent):
    ...
    def __init__(self, budget=100, name="Iql-PlayerAgent", cpa=2, category=0):
        ...
        for category_index in range(5):
            for budget_index in range(6):
                curr_model = IQL(dim_obs=16)
                curr_model.load_net(f"./saved_model/IQLtest{category_index}_{budget_index}")
                self.model_dict[f"{category_index}_{budget_index}"] = curr_model
        ...
        for category_index in range(5):
            for budget_index in range(6):
                with open(f'./saved_model/IQLtest{category_index}_{budget_index}/normalize_dict.pkl', 'rb') as file:
                    self.normalize_dict[f"{category_index}_{budget_index}"] = pickle.load(file)
    ...
    def action(self, tick_index, budget, remaining_budget, pv_values, history_pv_values, history_bid,
        ...
        """
        budget_index = int((budget - 1500) // 300)
        category_index = int(self.category)
        curr_key = f"{category_index}_{budget_index}"

        curr_model = self.model_dict[curr_key]
        curr_normalize_dict = self.normalize_dict[curr_key]
